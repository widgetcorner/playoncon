...
void main() {
  test('projects a control point near its mapped image position', () {
    // Pool control point → expected normalized image center ~ (0.5728, 0.3981)
    // on the current (numberless) map image.
    final p = MapGeoReference.instance.project(33.16675, -86.49314);
    expect(p, isNotNull);
    // Stylized map → allow the affine fit's residual (~0.04).
    expect((p!.dx - 0.5728).abs(), lessThan(0.06));
    expect((p.dy - 0.3981).abs(), lessThan(0.06));
  });
